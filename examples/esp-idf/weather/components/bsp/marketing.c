...
static void mic_speak_task(void *arg)
{
    ...
    const int BUF_SIZE = TOTAL_SAMPLES * BYTES_PER_SAMPLE * CHANNELS;

    uint8_t *psram_buf = (uint8_t *)heap_caps_malloc(BUF_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!psram_buf) {
    ...
    if (err == ESP_OK && i2c_handle != NULL) {
        ...
        if (err == ESP_OK) {
            ...
            ESP_LOGW(TAG, "qmi8658_init failed: %d", err);
            have_imu = false;
        }
    } else {
        ESP_LOGW(TAG, "bsp_i2c unavailable or init failed");
        have_imu = false;
    }

    // 角度阈值，60度
    const float flip_angle_deg = 60.0f;
    const float gyro_motion_threshold = 1.5f; // rad/s
    bool flipped = false;
    bool last_flipped = false;
    enum { WAIT_FLIP_TO_RECORD, RECORDING, WAIT_FLIP_TO_PLAY, PLAYING } state = WAIT_FLIP_TO_RECORD;
    ...
    while (1) {
        ...
        float gyro_motion = 0;
        float pitch = 0;
        if (have_imu && qmi8658_is_data_ready(&imu_dev, &ready) == ESP_OK && ready) {
            if (qmi8658_read_accel(&imu_dev, &ax, &ay, &az) == ESP_OK) {
                pitch = atan2f(-ax, sqrtf(ay * ay + az * az)) * 180.0f / 3.1415926f;
                if (pitch < 0) pitch = -pitch;
            }
            (void)qmi8658_read_gyro(&imu_dev, &gx, &gy, &gz);
            gyro_motion = fabsf(gx) + fabsf(gy) + fabsf(gz);
        }
        // 翻转检测：pitch角度大于80度且角速度大于阈值
        flipped = (pitch > flip_angle_deg && gyro_motion > gyro_motion_threshold);

        switch (state) {
        case WAIT_FLIP_TO_RECORD:
            bsp_display_lock(0);
            lv_label_set_text(label, "Flip to start recording");
            bsp_display_unlock();
            if (flipped && !last_flipped) {
                state = RECORDING;
                total_read = 0;
                bsp_display_lock(0);
                lv_label_set_text(label, "Recording... 0/10 s");
                bsp_display_unlock();
            }
            break;
        case RECORDING:
            if (total_read < BUF_SIZE) {
                ...
                ret = i2s_channel_read(_rx_handle, psram_buf + total_read, to_read, &bytes_read, 1000);
                if (ret != ESP_OK) {
                    ESP_LOGE(TAG, "i2s read failed");
                    break;
                }
                total_read += bytes_read;
                int sec = (total_read / (SAMPLE_RATE * BYTES_PER_SAMPLE * CHANNELS));
                char info[64];
                snprintf(info, sizeof(info), "Recording... %d/%d s", sec, SECONDS);
                bsp_display_lock(0);
                lv_label_set_text(label, info);
                bsp_display_unlock();
            } else {
                bsp_display_lock(0);
                lv_label_set_text(label, "Flip to start play");
                bsp_display_unlock();
                state = WAIT_FLIP_TO_PLAY;
            }
            break;
        case WAIT_FLIP_TO_PLAY:
            if (flipped && !last_flipped) {
                state = PLAYING;
                total_written = 0;
                bsp_display_lock(0);
                lv_label_set_text(label, "Playing... 0/10 s");
                bsp_display_unlock();
            }
            break;
        case PLAYING:
            if (total_written < BUF_SIZE) {
                size_t to_write = BUF_SIZE - total_written;
                if (to_write > 4096) to_write = 4096;
                ret = i2s_channel_write(_tx_handle, psram_buf + total_written, to_write, &bytes_written, 1000);
                if (ret != ESP_OK) {
                    ESP_LOGE(TAG, "i2s write failed");
                    break;
                }
                total_written += bytes_written;
                int sec = (total_written / (SAMPLE_RATE * BYTES_PER_SAMPLE * CHANNELS));
                char info[64];
                snprintf(info, sizeof(info), "Playing... %d/%d s", sec, SECONDS);
                bsp_display_lock(0);
                lv_label_set_text(label, info);
                bsp_display_unlock();
            } else {
                for(int i = 0; i < 200; i++) {
                    uint8_t silent_buf[128] = {0};
                    size_t bw = 0;
                    i2s_channel_write(_tx_handle, silent_buf, sizeof(silent_buf), &bw, 100);
                }
                state = WAIT_FLIP_TO_RECORD;
            }
            break;
        }
...
void marketing_task(void *arg)
{
    ...
    // _task_ctx->task = NULL;
    // _task_ctx->is_completed = false;
    // _task_ctx->timeout_ticks = 100000;
