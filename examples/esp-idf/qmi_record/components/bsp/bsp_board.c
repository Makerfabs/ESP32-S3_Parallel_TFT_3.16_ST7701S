...
esp_err_t bsp_extra_audio_init(bsp_audio_handle_t *handle)
{
    esp_err_t ret = ESP_OK;
    ...
    ret = bsp_audio_init(&_audio_handle, true);
    if(ret != ESP_OK) {
        ESP_LOGE(TAG, "bsp audio init failed");
        return ret;
    }

    if(_audio_handle) *handle = _audio_handle;

    _is_audio_init = true;

    ESP_LOGI(TAG, "bsp extra audio init done");
...
esp_err_t bsp_extra_player_init(void)
{
    ...
    if(!_is_audio_init) {
        ESP_LOGE(TAG, "bsp extra audio not init");
        return ESP_FAIL;
    }

    audio_player_config_t config = {
        .mute_fn    = _audio_handle->mute,
        .write_fn   = _audio_handle->write,
        .clk_set_fn = _audio_handle->set_output_format,
        .priority   = 5,
    };
    ...
    ESP_LOGI(TAG, "bsp extra player init done");
...
audio_player_state_t bsp_extra_player_get_state()
{
...
// static void bsp_extra_qmi8658_task(void *arg)
// {
//     while(true) {
...
//         bool is_ready = false;
//         float ax, ay, az;
//         if(_qmi8658_handle->read_accel(&ax, &ay, &az) == ESP_OK) {
//             qmi8658_orientation_e orientation = _qmi8658_handle->detect_orientation(ax, ay, az);
//         }
//     }
// }


esp_err_t bsp_extra_qmi8658_init(bsp_qmi8658_handle_t *handle)
{
    esp_err_t ret = ESP_OK;

    if(i2c_initalized == false) {
        bsp_i2c_init();
    }
    ...
    ret = bsp_qmi8658_init(&_qmi8658_handle, i2c_bus_handle, QMI8658_ADDRESS_HIGH);
    if(ret != ESP_OK) {
        ESP_LOGE(TAG, "bsp qmi8658 init failed");
        return ret;
    }

    if(_qmi8658_handle) *handle = _qmi8658_handle;

    // xTaskCreate(bsp_extra_qmi8658_task, "qmi8658_task", 4096, NULL, 5, NULL);
